...
*/
?>
<h2><?php printf( __( 'Get <em>another</em> %s site in seconds', 'easy-user-profile' ), $current_site->site_name ); ?></h2>

<?php if ( $errors->get_error_code() ) { ?>
	<p><?php _e( 'There was a problem, please correct the form below and try again.', 'easy-user-profile' ); ?></p>
<?php } ?>

<p><?php printf( __( 'Welcome back, %s. By filling out the form below, you can <strong>add another site to your account</strong>. There is no limit to the number of sites you can have, so create to your heart&#8217;s content, but write responsibly!', 'easy-user-profile' ), $current_user->display_name ); ?></p>

<?php
$blogs = get_blogs_of_user( $current_user->ID );
if ( ! empty( $blogs ) ) { ?>
		<p><?php _e( 'Sites you are already a member of:', 'easy-user-profile' ); ?></p>
		<ul>
...
<p><?php _e( 'If you&#8217;re not going to use a great site domain, leave it for a new user. Now have at it!', 'easy-user-profile' ); ?></p>
<form id="setupform" method="post" action="<?php $template->the_action_url( 'register', 'login_post' ); ?>">
	...
	<p class="submit"><input type="submit" name="submit" class="submit" value="<?php esc_attr_e( 'Create Site', 'easy-user-profile' ); ?>" /></p>
</form>
