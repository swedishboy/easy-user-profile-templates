...
class Theme_My_Login_Custom_Passwords extends Theme_My_Login_Abstract {
	...
	public function password_fields() {
		...
		<p class="tml-user-pass1-wrap">
			<label for="pass1<?php $template->the_instance(); ?>"><?php _e( 'Password', 'easy-user-profile' ); ?></label>
		...
		<p class="tml-user-pass2-wrap">
			<label for="pass2<?php $template->the_instance(); ?>"><?php _e( 'Confirm Password', 'easy-user-profile' ); ?></label>
	...
	public function ms_password_fields() {
		...
		<span class="hint"><?php echo apply_filters( 'tml_password_hint', sprintf( __( '(Must be at least %d characters.)', 'easy-user-profile' ), apply_filters( 'tml_minimum_password_length', 6 ) ) ); ?></span>

		<label for="pass2<?php $template->the_instance(); ?>"><?php _e( 'Confirm Password:', 'easy-user-profile' ); ?></label>
		<input autocomplete="off" name="pass2" id="pass2<?php $template->the_instance(); ?>" class="input" size="20" value="" type="password" /><br />
		<span class="hint"><?php echo apply_filters( 'tml_password_confirm_hint', __( 'Confirm that you\'ve typed your password correctly.', 'easy-user-profile' ) ); ?></span>
		<?php
	}
	...
	public function password_errors( $errors = '' ) {
		...
		if ( empty( $_POST['pass1'] ) || empty( $_POST['pass2'] ) ) {
			$errors->add( 'empty_password', __( '<strong>ERROR</strong>: Please enter your password twice.', 'easy-user-profile' ) );

		// Make sure there's no "\" in the password
		} elseif ( false !== strpos( stripslashes( $_POST['pass1'] ), "\\" ) ) {
			$errors->add( 'password_backslash', __( '<strong>ERROR</strong>: Passwords may not contain the character "\\".', 'easy-user-profile' ) );

		// Make sure passwords match
		} elseif ( $_POST['pass1'] != $_POST['pass2'] ) {
			$errors->add( 'password_mismatch', __( '<strong>ERROR</strong>: Please enter the same password in the two password fields.', 'easy-user-profile' ) );

		// Make sure password is long enough
		} elseif ( strlen( $_POST['pass1'] ) < apply_filters( 'tml_minimum_password_length', 6 ) ) {
			$errors->add( 'password_length', sprintf( __( '<strong>ERROR</strong>: Your password must be at least %d characters in length.', 'easy-user-profile' ), apply_filters( 'tml_minimum_password_length', 6 ) ) );
	...
	public function action_messages( &$theme_my_login ) {
		...
		if ( isset( $_GET['registration'] ) && 'complete' == $_GET['registration'] )
			$theme_my_login->errors->add( 'registration_complete', __( 'Registration complete. You may now log in.', 'easy-user-profile' ), 'message' );
	}
